...
func TestGRPCClient_RemoveUser_Success(t *testing.T) {
	...
	err := c.RemoveUser("test@example.com", []string{"vless-tag", "vmess-tag"})
	if err != nil {
...
func TestGRPCClient_RemoveUser_EmptyTags(t *testing.T) {
	...
	err := c.RemoveUser("test@example.com", nil)
	if err != nil {
...
func TestGRPCClient_RemoveUser_ServerError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "tag1") {
		t.Errorf("error should mention failing tag, got: %v", err)
	}
}
...
func TestGRPCClient_AddUser_Success(t *testing.T) {
	...
	err := c.AddUserSingle("inbound-vless", "vless", clientJSON)
	if err != nil {
...
func TestGRPCClient_AddUser_InvalidProtocol(t *testing.T) {
	c := newTestGRPCClient("127.0.0.1:1")
	err := c.AddUserSingle("tag1", "invalid_proto", []byte(`{}`))
	if err == nil {
