...
func TestCoreInitPublishesAllDeclaredServices(t *testing.T) {
	...
	require.IsType(t, registry, services[pluginapi.ServiceDomainRegistry])

	_, ok := services[pluginapi.ServiceDomainRegistry].(domain.Registry)
	require.True(t, ok)
	_, ok = services[pluginapi.ServiceUserRepository].(pluginapi.UserRepository)
	require.True(t, ok)
	_, ok = services[pluginapi.ServiceSubscriptionRepository].(pluginapi.SubscriptionRepository)
	require.True(t, ok)
	_, ok = services[pluginapi.ServiceDeviceRepository].(pluginapi.DeviceRepository)
	require.True(t, ok)
	_, ok = services[pluginapi.ServicePlanRepository].(pluginapi.PlanRepository)
	require.True(t, ok)
...
func TestHostLoadsAndExposesCoreProvider(t *testing.T) {
	...
	service, err := host.ResolveService(pluginapi.ServiceUserRepository)
	require.NoError(t, err)
