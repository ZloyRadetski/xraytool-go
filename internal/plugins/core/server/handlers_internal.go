...
func (r *Router) handleInternalXraySync(w http.ResponseWriter, req *http.Request) {
	...
	case "antifraud-events":
		...
		events := make([]pluginapi.FraudEvent, len(payloadReq.Events))
		for i, event := range payloadReq.Events {
			events[i] = pluginapi.FraudEvent{Email: event.Email, IP: event.IP}
		}
		if err := r.ingestEvents(req.Context(), getClientIP(req), events); err != nil {
			r.log.Error("internal sync: antifraud events rejected", "err", err)
