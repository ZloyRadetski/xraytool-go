...
func (r *Router) handleSyncSnapshot(w http.ResponseWriter, req *http.Request) {
	...
	users, err := r.clusterSync.BuildSnapshot(req.Context())
	if err != nil {
...
func (r *Router) handleSyncState(w http.ResponseWriter, req *http.Request) {
	...
	state, err := r.clusterSync.MasterState(req.Context())
	if err != nil {
...
// WithSyncService is a compatibility bridge for callers that still provide a
// legacy sync service. New code must use WithClusterSyncProvider. The
// untyped value keeps package server independent of internal/statesync while
// retaining slave-side registry wiring during the migration.
func (r *Router) WithSyncService(provider any, registry domain.Registry) *Router {
	r.registry = registry
	if cluster, ok := provider.(pluginapi.ClusterSyncProvider); ok {
		r.WithClusterSyncProvider(cluster)
	}
	return r
}
