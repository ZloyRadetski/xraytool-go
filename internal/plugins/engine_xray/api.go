...
func (c *Client) AddUser(ctx context.Context, payload []TaggedClient, configPath string) error {
	if len(payload) == 0 {
		return nil
	}
...
func (c *Client) RemoveUser(ctx context.Context, email string, tags []string) error {
	if len(tags) == 0 {
		return nil
	}
...
func (c *Client) QueryStats(ctx context.Context) ([]UserStat, error) {
	callCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
...
func buildAddPayload(payload []TaggedClient, configPath string) aduPayload {
	...
	ibByTag := make(map[string]RawInbound, len(inbounds))
