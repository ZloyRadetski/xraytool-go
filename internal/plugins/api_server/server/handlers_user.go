...
func (r *Router) handleAdjustBalance(w http.ResponseWriter, req *http.Request) {
	...
	if body.Amount == 0 || body.Amount < -100000000 || body.Amount > 100000000 {
		writeError(w, http.StatusBadRequest, "amount must be non-zero and between -100000000 and 100000000")
		return
	}
...
func (r *Router) handleAdminUnblockUser(w http.ResponseWriter, req *http.Request) {
	...
	if r.forceUnban != nil {
		if err := r.forceUnban(req.Context(), email); err != nil {
			r.log.Warn("admin unblock user: antifraud force-unban failed", "email", email, "err", err)
		}
	}
...
func (r *Router) handleAdminAntiFraudState(w http.ResponseWriter, req *http.Request) {
	...
	snapshot, err := r.getSnapshot(req.Context())
	if err != nil {
		r.log.Error("admin antifraud state: provider snapshot failed", "err", err)
		writeError(w, http.StatusBadGateway, "antifraud provider unavailable")
		return
	}
	response := make(map[string]interface{}, len(snapshot)+1)
