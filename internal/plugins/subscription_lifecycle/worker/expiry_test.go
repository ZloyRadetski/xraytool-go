...
func TestExpiryWorker_BatchMixedUsers(t *testing.T) {
	...
	db.Create(&database.Subscription{ID: "sub-exp", UserID: "u1", UUID: "uuid1", Email: "exp@x.com", Status: "active", EndsAt: &endsExpired})
	db.Create(&database.Subscription{ID: "sub-soon", UserID: "u2", UUID: "uuid2", Email: "soon@x.com", Status: "active", EndsAt: &endsSoon})
	db.Create(&database.Subscription{ID: "sub-late", UserID: "u3", UUID: "uuid3", Email: "late@x.com", Status: "active", EndsAt: &endsLate})
