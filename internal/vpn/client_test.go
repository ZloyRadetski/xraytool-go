...
func TestClient_AddUser(t *testing.T) {
	...
	if err := c.AddUser(context.Background(), payload, cfgPath); err != nil {
		t.Errorf("expected no error, got %v", err)
	}

	// 2. Empty payload case
	if err := c.AddUser(context.Background(), nil, cfgPath); err != nil {
		t.Errorf("expected no error, got %v", err)
	}
	...
	payloadWithFail := []TaggedClient{
		{Tag: "tag1", Client: rc},
		{Tag: "failtag", Client: rc},
	}

	err := c.AddUser(context.Background(), payloadWithFail, cfgPath)
	if err == nil {
...
func TestClient_RemoveUser(t *testing.T) {
	...
	if err := c.RemoveUser(context.Background(), "test@example.com", []string{"tag1", "tag2"}); err != nil {
		t.Errorf("expected no error, got %v", err)
	}
