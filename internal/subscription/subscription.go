...
type DeviceState struct {
	Clients map[string]*ClientDevices `json:"clients"`
}

// ClientDevices holds devices tracked for a single client key.
type ClientDevices struct {
	Devices []DeviceItem `json:"devices"`
}

// DeviceItem represents a single client device.
...
// Process parses the request, updates device states, and generates subscription response.
func Process(cm *CacheManager, req *Request) *Response {
	...
	// 3. Load Xray Config and DBs from Cache
	cm.Refresh() // Smart refresh based on modTime (noop if not modified)

	activeUser, limitedUser := cm.GetUserBySubfile(filename)
	...
	if activeUser == nil && limitedUser == nil {
		return failResponse(404, "Subscription not found")
	}

	isBlockedUser := activeUser == nil
	email := ""
	uuid := ""
	userPassSs := ""
	expireVal := ""
	...
	if isBlockedUser {
		...
		expireVal = defaultExpire
		deviceLimit = 0
	} else {
		email = activeUser.Email
		uuid = activeUser.ID
		userPassSs = activeUser.Password
		expireVal = activeUser.Expire
		rawHy2Auth = activeUser.Hy2Auth
		deviceLimit = activeUser.Limit
	}
	...
	if !skipChecks {
		...
		if !unsupportedClient && hwid != "" {
			// Update devices state in CacheManager
			var err error
			deviceLimitReached, err = cm.UpdateDeviceState(filename, clientId, hwid, deviceLimit, deviceModel, deviceOs, verOs, req.UserAgent)
			if err != nil {
				return failResponse(500, fmt.Sprintf("device storage error: %v", err))
			}
			if deviceLimitReached {
				...
				if strings.HasPrefix(email, "bot_client_") {
					if tgID, err := strconv.ParseInt(strings.TrimPrefix(email, "bot_client_"), 10, 64); err == nil {
	...
	// Fetch traffic download bytes (tries inferred_stats first, then falls back to stats_state)
