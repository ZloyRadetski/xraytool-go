...
- Статические (захардкоженные) пользователи шаблона передаются как профили пользователей, а не как копия inbound по тегу master. Поэтому теги и даже наборы inbound на узлах могут различаться: каждый такой пользователь добавляется во все совместимые локальные inbound slave-узла. Master только публикует свой артефакт и никогда не применяет его к собственному конфигу. Неподписанные записи без `email` не считаются пользователями и сохраняются только при точном совпадении локального тега и протокола.
