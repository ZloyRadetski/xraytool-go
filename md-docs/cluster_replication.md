...
- Ключи Reality остаются отдельным артефактом конфигурации и передаются только по mTLS-потоку; на slave они записываются атомарно с правами `0600`.
- Захардкоженные пользователи из Xray template на master читаются без записи в файл, преобразуются в обычные профили пользователей и добавляются к снимку БД. Для slave они не отличаются от пользователей БД: попадают в те же user-frames и применяются тем же `SyncUsers`.
- Template служит только структурным каркасом. При восстановлении slave все записи с `email` строятся из снимка master; локальные template-пользователи, которых в снимке нет, не сохраняются. Сам template никогда не изменяется, а `config.json.static-clients.json` больше не используется.
- Источник template-пользователей выбирается только при единственном поддерживающем движке (обычно `engine_xray`). При двух таких движках репликация не строит неоднозначный снимок, пока в протокол не добавлен идентификатор движка.
...
- На master доступны команды `xraytool plugin run cluster_replication status` и `xraytool plugin run cluster_replication snapshot`. `snapshot` добавляет компактный маркер, а подключённые slave сами получают развёрнутый потоковый снимок. `xraytool syncstates --full` пересобирает master из template и полного снимка (БД плюс template-пользователи), затем отправляет этот же снимок slave.
